...
const Chefcard = ({ data }) => {
  ...
  return (
    <div data-aos="zoom-in-up" data-aos-duration="2000"  className="card  md:card-side bg-base-100 shadow-xl my-5 md:mx-16">
      ...
      <div className="card-body">
        ...
        <span className='md:flex items-center'><FaRegThumbsUp /> {likes}</span>
          <Link to={`/chefs/${code}`}><button className='shadow-lg hover:border-2 font-medium p-2 rounded-md'>See Details</button></Link>
        </div>
      </div>
    </div>
  );
